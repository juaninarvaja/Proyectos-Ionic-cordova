...
export class FotosService {
  ...
  /** Esta no es de firebase, es un filtrado de fotos */
  public FiltrarFotosMias(fotos: Array<Foto>, tipo: string): Array<Foto> {
    const auxReturn = new Array<Foto>();
    ...
    for (const foto of fotos) {
      if (foto.tipo === tipo) {
        if(foto.usuario === auxuser.toString())
        {
          auxReturn.push(foto);
        }
      }
...
/** conecto  con Cloud firestore para subir el archivode la foto*/
// public UploadToFirebaseStorage(image:any, firestoreDB:string) {
//   const imgBase64 = `data:image/jpeg;base64,${image}`;
//   const user = `${this.MiAuth.auth.currentUser.email.split('@')[0]}`;
//   const refImage = `${this.MiAuth.auth.currentUser.email.split('@')[0]}_${(new Date()).getTime()}`;
//   const url = this.fireStorage.storage.ref(refImage).putString(imgBase64,'data_url');
//   const type = firestoreDB === 'Lindas' ? 'L' : 'F';
//   this.addCloudFirestore(refImage,user,firestoreDB,type,imgBase64, new Date().getTime());
// }

// private addCloudFirestore(idImg,user, firestoreDB, type, imgBase64, fecha) {
//   this.firestore.collection(firestoreDB).add({
//     id:idImg ,
//     userName: user ,
//     typeImg: type , 
//     image: imgBase64 ,
